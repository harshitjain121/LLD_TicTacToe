...
public class OrderOneGameWinningStrategy implements GameWinningStrategy {
    ...
    public boolean checkWinner(Board board, Move move) {
        ...
        if (isCellOnTopLeftDiagonal(row, col)) {
            ...
            topLeftDiaganolSymbolCounts.put(symbol, topLeftDiaganolSymbolCounts.get(symbol) + 1);
        }
        ...
        if (isCellOnTopRightDiagonal(row, col, dimension)) {
            ...
            topRightDiaganolSymbolCounts.put(symbol, topRightDiaganolSymbolCounts.get(symbol) + 1);
        }
        ...
        if (isCellOnTopLeftDiagonal(row, col) && topLeftDiaganolSymbolCounts.get(symbol) == dimension) {
            return true;
        }

        if (isCellOnTopRightDiagonal(row, col, dimension) && topRightDiaganolSymbolCounts.get(symbol) == dimension) {
            return true;
        }
